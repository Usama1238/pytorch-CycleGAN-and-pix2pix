...
class Identity(nn.Module):
    def forward(self, x):
        return x
def init_net1(net1, init_type='normal', init_gain=0.02, gpu_ids=[], debug=False, initialize_weights=True):
    ...
    if initialize_weights:
        init_weights1(net1, init_type, init_gain=init_gain,debug=debug)
    return net1    
...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[], opt=None):
    if netF == 'global_pool':
        ...
        net1 = PatchSampleF(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc)
    elif netF == 'strided_conv':
        net = StridedConvF(init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids)
    else:
        raise NotImplementedError('projection model name [%s] is not recognized' % netF)
    return init_net1(net1, init_type, init_gain, gpu_ids)
