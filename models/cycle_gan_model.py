...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B','NCE_loss']
    ...
    def backward_G(self):
        ...
        if self.opt.lambda_GAN > 0.0:
            self.loss_G_B = self.criterionGAN(self.netD_B(self.fake_A), True).mean() * self.opt.lambda_GAN
        else:
            self.loss_G_GAN = 0.0
    ...
    def calculate_NCE_loss(self, fake_B, rec_A1):
        n_layers = len(self.nce_layers)
        feat_q = self.netG_B(rec_A1, self.nce_layers, encode_only=True)
        #if self.opt.flip_equivariance and self.flipped_for_equivariance:
            #feat_q = [torch.flip(fq, [3]) for fq in feat_q]
        feat_k = self.netG_B(fake_B, self.nce_layers, encode_only=True)
        #print(feat_k)
